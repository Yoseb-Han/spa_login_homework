...
router.post("/:postId", async (req, res) => {
    try {
        ...
        res.status(200).json({ errorMessage: "댓글을 작성하였습니다." });
    } catch (error) {
        console.error(error);
...
router.get("/:postId", authMiddleware, async (req, res) => {
    try {
        ...
        if (existPosts.length) {
            ...
            return res
                .status(400)
                .json({ errorMessage: "존재하지 않는 게시글의 댓글입니다." });
        }
    } catch (error) {
        console.error(error);
        res.status(400).json({
            errorMessage: "댓글 목록 조회에 실패하였습니다.",
        });
    }
});
...
router.put("/:commentId", async (req, res) => {
    ...
    await Comments.update({ comment }, { where: { commentId, userId } });
